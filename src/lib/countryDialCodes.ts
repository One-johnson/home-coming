...
/**
 * ITU-T E.164 calling codes with ISO 3166-1 alpha-2 for flag SVGs.
 * Sorted by country name; dial codes are stored with a leading "+".
 */
...
/** Prefer primary territories when several share a dial code. */
const DIAL_CODE_PREFERRED_ISO: Record<string, string> = {
  "+1": "US",
  "+7": "RU",
  "+44": "GB",
  "+262": "RE",
...
export function countryFlagSvgUrl(iso2: string) {
...
export function findCountryByDialCode(
  ...
  const matches = COUNTRY_DIAL_CODES.filter((c) => c.dialCode === normalized);
  if (matches.length === 0) return undefined;
  if (matches.length === 1) return matches[0];

  if (preferredIso) {
    const preferred = matches.find(
      (c) => c.iso2.toLowerCase() === preferredIso.toLowerCase(),
    );
    if (preferred) return preferred;
  }

  const defaultIso = DIAL_CODE_PREFERRED_ISO[normalized];
  if (defaultIso) {
    const preferred = matches.find((c) => c.iso2 === defaultIso);
    if (preferred) return preferred;
  }
...
const REGION_PREFERRED_ISO: Record<string, string> = {
  ghana: "GH",
  usa: "US",
  canada: "CA",
  uk: "GB",
...
/** Hint which flag to show when a dial code is shared (e.g. +1). */
export function preferredIsoForRegion(regionKey: string): string | undefined {
  return REGION_PREFERRED_ISO[regionKey];
}
